...
def makeInput(imgName):
    ...
    # patches_list = os.listdir('./early-1015')
    ...
    for idx, item in enumerate(patches_list):
        ...
        # patch = cv2.imread('./early-1015/{}'.format(item), 0)
...
if __name__ == '__main__':
    ...
    for imgName in names:
        ...
        #     # cam_show_img(patchImg.detach().cpu().numpy(), fmap, grads_val, '{}.jpg'.format(idx.__str__()))
        #     cam_show_img(patchImg.detach().cpu().numpy(), fmap, grads_val, '31.jpg')
        cam_show_img(img, fmap, grads_val, '31.jpg')
        print(imgName, ' done')

        # for idx in range(loc_imgs.shape[1]):
        #     p_patch = l_out[idx]
        #     net.zero_grad()
        #     loss = p_patch[0, p_patch.argmax()]
        ...
        #     cam_show_img(patchImg.detach().cpu().numpy(), fmap, grads_val, '{}.jpg'.format(idx.__str__()))
        #     print(imgName, ' done')
        #     grads_val = grads_val.tolist().clear()
        #     fmap = fmap.tolist().clear()
